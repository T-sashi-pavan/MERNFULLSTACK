...
exports.join = (req, res) => {
    User.aggregate([
        {
            $lookup: {
                from: 'Department', // Ensure this is the correct collection name in MongoDB
                localField: 'did',
                foreignField: 'did',
                as: 'department'
            }
        ...
        {
            $project: {
                _id: 0,               // Optional: to exclude the _id field from the output
                uid: 1,
                uname: 1,
                'department.dname': 1 // Use 'department.dname' to access the department name
            }
        }
    ])
    .then(results => {
